...
def broadcast_to_lobby(lobby_name, channel_db_id, payload, note=""):
    ...
    with sessions_lock:
        for session in sessions.values():
            if session.get("player_id") in player_ids:
                send_packet_to_client(session, payload, note=f"[BROADCAST {lobby_name}] {note}")
